...
//for each category
const discountProduct=ProductStore.getDiscountProduct();

const currentPage = ref(1);
const itemsPerPage = 8;

const totalPages = computed(() => Math.ceil(discountProduct.length / itemsPerPage));

const paginatedProducts = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return discountProduct.slice(start, start + itemsPerPage);
});

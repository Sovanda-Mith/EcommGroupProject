...
//for each category
const budgetProduct=ProductStore.getBudgetProduct();

const currentPage = ref(1);
const itemsPerPage = 8;

const totalPages = computed(() => Math.ceil(budgetProduct.length / itemsPerPage));

const paginatedProducts = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return budgetProduct.slice(start, start + itemsPerPage);
});

...
//for each category
const otherProduct=ProductStore.getProductByCategory('Other');

const currentPage = ref(1);
const itemsPerPage = 8;

const totalPages = computed(() => Math.ceil(otherProduct.length / itemsPerPage));

const paginatedProducts = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return otherProduct.slice(start, start + itemsPerPage);
});

...
//for each category
const hotProduct=ProductStore.getHotProduct();

const currentPage = ref(1);
const itemsPerPage = 8;

const totalPages = computed(() => Math.ceil(hotProduct.length / itemsPerPage));

const paginatedProducts = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return hotProduct.slice(start, start + itemsPerPage);
});

...
// });
//for each category
const monitorProduct = ProductStore.getProductByCategory('Monitor');

const currentPage = ref(1);
const itemsPerPage = 5;

const totalPages = computed(() => Math.ceil(monitorProduct.length / itemsPerPage));

const paginatedProducts = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return monitorProduct.slice(start, start + itemsPerPage);
});
